...
clockwise-arrows emoji (see ``_REPOST_EMOJIS``) specifically is
this bridge's repost shorthand instead -- sent as a real ``Announce`` (see
``send_repost``, also shared with the ``;repost`` command in
``bridge.commands``) rather than a Like/EmojiReact, fanned out to the
reposter's own followers AND delivered directly to the original author.

Also handles the reverse: a Matrix redaction of a reaction the bridge itself
sent out earlier is federated as an ``Undo``, so un-reacting (or un-reposting)
works too. Both directions record a ``ReactionRecord`` (see
``bridge.repository``) so the two can find each other -- this applies just
as well to a repost recorded via the ``;repost`` command, keyed to that
...
# suggestions, and also the emoji this bridge's own repost renderings
# are prefixed with (see bridge.commands._handle_repost,
# bridge.reaction_bridge.send_repost), so reacting with the same symbol the
# bridge itself already uses to mean "reposted" reads as the more
# discoverable/obvious choice. The recycling symbol (♻) used to also work
# here but was dropped -- keeping just one canonical repost emoji avoids any
# ambiguity about which reaction "really" means repost.
_REPOST_EMOJIS = ("\U0001F501",)
...
def _is_repost_emoji(key: str) -> bool:
    """Whether ``key`` is one of ``_REPOST_EMOJIS`` (optionally followed by
...
# action: the 🔁 reaction (which alone already triggers send_repost below)
# and a separate org.matrix.msc4501.social.relates_to record an MSC4501
# client posts into the reposter's OWN Profile Room (see
# bridge.profile_posts._maybe_suppress_msc4501_repost_record). When that
# record lands in the SAME room send_repost would post its own "you
# reposted" card into, the two are redundant -- Haven's own native record
# already shows the reposted content there. Arrival order between the two
...
# (reposter's own matrix user id, the REPOSTED post's own matrix event id --
# available as ``parent.event_id`` in send_repost, and directly as
...
async def note_msc4501_native_repost(request: Request, *, reposter_matrix_user_id: str, reposted_event_id: str) -> None:
    ...
    if now - posted_at > _REPOST_CARD_DEDUP_WINDOW_SECONDS:
        return  # too long ago to safely assume it's the SAME repost action
...
async def send_repost(
    ...
    *,
    ...
    "repost"/"boost" in Mastodon's own terms) signed as ``actor_record``.
    ...
    and a caption-less ``;repost``/``;boost`` command (see
    ``bridge.commands._handle_repost``) -- they only differ in what
    ``matrix_event_id`` ends up being (the reaction event itself, vs. the
    command message that triggered it).

    Also posts a "X reposted Y's post" rendering (bold names, pills -- see
    ...
    docstring), so redacting EITHER one works to undo the repost: this is
    ...
    entirely, and the person who reposted may not remember/still have
    ...
    # reposter, not the actual post being reposted -- reposting THAT
    # message again should still reach (and re-repost) the original
    # post/author, not the intermediate repost.
    ...
    if actor_record.room_id and not already_has_native_record:
        ...
        # repost card showed neither the caption nor a working embed).
        ...
        try:
            ...
            # If the MSC4501 native record for this SAME repost shows up in
...
async def maybe_federate_reaction(request: Request, event: dict) -> bool:
    ...
    if _is_repost_emoji(key):
        # A repost is its own activity type (Announce), not a Like/EmojiReact
        # variant -- see send_repost, shared with a caption-less ";repost"
        # (or its ";boost" alias). send_repost already posts a visible "you
        # reposted so-and-so's post" record into the reposter's own Profile
        # Room -- no separate confirmation here on top of that.
        await send_repost(
            request,
    ...
    # the Announce activity and its reposter (see FederatedEvent's
    # docstring) -- a reaction to that message needs to target the actual
    # reposted post and reach its actual author, not "like" someone's repost
    ...
    if matrix_event_id:
        ...
        # FederatedEvent (the reposted post's own, from its ALSO having
        # been actually imported -- see _handle_announce) and re-derives
        # the same real author, not the reposter.
        await repository.record_reaction(
            ReactionRecord(
                activity_id=activity_id,
                room_id=room_id,
...
async def maybe_federate_reaction_removal(request: Request, event: dict) -> bool:
    ...
    # author_actor_id is the reposter, not the reposted post's real author
    # who actually received the original reaction and needs the Undo.
    target_author_actor_id = parent.reposted_author_actor_id or parent.author_actor_id
    ...
    # An Announce (see send_repost) is ALSO fanned out to the reposter's own
    ...
    # timeline keeps showing a "repost" that's since been retracted. Told
    # apart from a Like/EmojiReact's activity_id by send_repost's own
